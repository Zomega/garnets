...
from enviroment import gravity
...
def generate_planetary_masses(star, inner_dust, outer_dust, do_moons=True):
    ...
    while disk.dust_left and sequential_failures < 10**3:
        ...
        if disk.dust_available(iel, oel) > 0:
            sequential_failures = 0
            logging.info("Injecting planetesimal at " + str(canidate.orbit.a) +
                         " AU ...")
    ...
    for planet in planets:
        ...
        if abs(diff) <= abs(dist1) or abs(diff) <= abs(dist2):
            ...
            temp = planet.mass * sqrt(
                planet.orbit.a) * sqrt(1.0 - (planet.orbit.e**2.0))
                    ...
                    if canidate.mass * SUN_MASS_IN_EARTH_MASSES < 2.5 \
                        ...
                        planet.add_moon(
                            convert_planetesimal_to_protomoon(
                                canidate, planet))
                        logging.info(
                            "Moon captured at " + str(planet.orbit.a) +
                            " AU. Planet Mass: " +
                            str(planet.mass * SUN_MASS_IN_EARTH_MASSES) +
                            " earth masses Moon Mass: " +
                            str(canidate.mass * SUN_MASS_IN_EARTH_MASSES) +
                            " earth masses.")
                        finished = True
                        break
                    else:
            ...
            finished = True
            logging.info("Conglomerate is now " +
                         str(planet.mass * SUN_MASS_IN_EARTH_MASSES) +
...
def calculate_gases(star, planet, planet_id):
    if planet.surf_pressure > 0:
        ...
        for i in range(len(gases)):
            ...
            if ((yp >= 0 and yp < planet.low_temp)
                ...
                ) and star.age > 2e9 and planet.surf_temp > 270 and planet.surf_temp < 400:
                    pres2 = (0.89 + pressure / 4)
                    react = pow(1 / (1 + gases[i].reactivity),
                                pow(star.age / 2e9, 0.25) * pres2)

                elif gases[
                        i].symbol == "CO2" and star.age > 2e9 and planet.surf_temp > 270 and planet.surf_temp < 400:
                    pres2 = (0.75 + pressure)
                    react = pow(1 / (1 + gases[i].reactivity),
                                pow(star.age / 2e9, 0.5) * pres2)
                    react *= 1.5
...
def roche_limit(planet, moon):
    return 2.44 * planet.radius * pow((planet.density / moon.density),
                                      (1.0 / 3.0))


def hill_sphere(planet, star):
    return planet.orbit.a * KM_PER_AU * pow(
        (planet.mass / (3.0 * star.mass_ratio)), (1.0 / 3.0))
