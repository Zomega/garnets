...
class PlanetType(Enum):
  ...
  ICE = 9
  ASTERIODS = 10
  ONE_FACE = 11  # TODO(woursler): Don't know what this means... maybe tidally locked?
...
def kothari_radius(mass, giant, zone):
  ...
  temp = temp / JIMS_FUDGE;      ''' Earth = actual earth '''
...
def day_length(planet):
  ...
  change_in_angular_velocity = CHANGE_IN_EARTH_ANG_VEL * (planet.density / EARTH_DENSITY) * (equatorial_radius_in_cm / EARTH_RADIUS) * (EARTH_MASS_IN_GRAMS / planetary_mass_in_grams) * pow(planet.sun.mass, 2.0) * (1.0 / pow(planet.a, 6.0))
...
def calculate_surface_temp(planet, first, last_water, last_clouds, last_ice, last_temp, last_albedo):
  ...
  if planet.greenhouse_effect and planet.max_temp < planet.boil_point:
    if flag_verbose & 0x0010: # TODO(woursler)
