...
long double day_length(planet_pointer  planet)
  long planetary_mass_in_grams = planet.mass * SOLAR_MASS_IN_GRAMS
...
long double rms_vel(long double molecular_weight, double exospheric_temp)
...
int zone, greenhouse_effect, accreted_gas
  ...
  if velocity_ratio >= GAS_RETENTION_THRESHOLD:
    switch(zone)      case 1:
      ...
      default:
        proportion_ = 0.0
        printf("Error: orbital zone not initialized correctlynot \n")
        break
...
'''   surface covered by water is 71%, 75% as Fogg used.          '''
'''--------------------------------------------------------------------------'''
...
def grnhouse(self, double r_ecosphere, double orb_radius):
  long temp = eff_temp(r_ecosphere, orb_radius, GREENHOUSE_TRIGGER_ALBEDO)
...
long double optical_depth, effective_temp, surf_pressure
  long convection_factor = EARTH_CONVECTION_FACTOR * 
                  pow(surf_pressure / 
                    EARTH_SURF_PRES_IN_MILLIBARS, 0.4)
  long rise = (pow1_4(1.0 + 0.75 * optical_depth) - 1.0) * 
...
long double gas_life(long double molecular_weight, 
  ...
  long v = rms_vel(molecular_weight, planet.exospheric_temp)
  long g = planet.surf_grav * EARTH_ACCELERATION
  long r = (planet.radius * CM_PER_KM)
  long t = (pow3(v) / (2.0 * pow2(g) * r)) * exp((3.0 * g * r) / pow2(v))
  long years = t / (SECONDS_PER_HOUR * 24.0 * DAYS_IN_A_YEAR)
...
long double min_molec_weight (planet_pointer planet)
  long mass = planet.mass
  long radius = planet.radius
  long temp = planet.exospheric_temp
  long target = 5.0E9
  
  long guess_1 = molecule_limit (mass, radius, temp)
  long guess_2 = guess_1
  
  long life = gas_life(guess_1, planet)
  ...
  while (((guess_2 - guess_1) > 0.1) and (loops++ < 25))
    long guess_3 = (guess_1 + guess_2) / 2.0
...
void calculate_surface_temp(planet_pointer   planet,
  ...
  if (planet.greenhouse_effect
   and planet.max_temp < planet.boil_point)
  ...
  if (planet.greenhouse_effect:
   and (planet.surf_pressure > 0.0))
    planet.cloud_cover  = 1.0
  
  if (planet.high_temp >= planet.boil_point:
   and (not first)
   and not ((int)planet.day == (int)(planet.orb_period * 24.0) or
...
def iterate_surface_temp(self, planet):
planet_pointer planet; 
  count = 0
  long initial_temp = est_temp(planet.sun.r_ecosphere, planet.a, planet.albedo)

  long h2_life = gas_life (MOL_HYDROGEN,    planet)
  long h2o_life = gas_life (WATER_VAPOR,     planet)
  long n2_life = gas_life (MOL_NITROGEN,    planet)
  long n_life = gas_life (ATOMIC_NITROGEN, planet)
  ...
  for (count = 0
     count <= 25
     count++)
    long last_water = planet.hydrosphere
    long last_clouds = planet.cloud_cover
    long last_ice = planet.ice_cover
    long last_temp = planet.surf_temp
    long last_albedo = planet.albedo
...
'''--------------------------------------------------------------------------'''
'''   Inspired partial pressure, into account humidification of the  '''
...
long double inspired_partial_pressure (long double surf_pressure,
  ...
  long pH2O = (H20_ASSUMED_PRESSURE)
  long fraction = gas_pressure / surf_pressure
...
unsigned int breathability (planet_pointer planet)
  ...
  for (index = 0; index < planet.gases; index++)
    ...
    long ipp = inspired_partial_pressure (planet.surf_pressure,
...
long double soft(long double v, double max, double min)
  long dv = v - min
  long dm = max - min
  return (lim(2*dv/dm-1)+1)/2 * dm + min


def set_temp_range(self, planet):
  long pressmod = 1 / sqrt(1 + 20 * planet.surf_pressure/1000.0)
  long ppmod = 1 / sqrt(10 + 5 * planet.surf_pressure/1000.0)
  long tiltmod = fabs(cos(planet.axial_tilt * PI/180) * pow(1 + planet.e, 2))
  long daymod = 1 / (200/planet.day + 1)
  long mh = pow(1 + daymod, pressmod)
  long ml = pow(1 - daymod, pressmod)
  long hi = mh * planet.surf_temp
  long lo = ml * planet.surf_temp
  long sh = hi + pow((100+hi) * tiltmod, sqrt(ppmod))
  long wl = lo - pow((150+lo) * tiltmod, sqrt(ppmod))
  long max = planet.surf_temp + sqrt(planet.surf_temp) * 10
  long min = planet.surf_temp / sqrt(planet.day + 24)
